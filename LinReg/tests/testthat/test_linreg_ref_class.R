...
test_that("lenreg rejects errounous input", {
    expect_error(linreg_mod <- linreg(formula = Petal.Length~Sepdsal.Width+Sepal.Length, data=iris))
    expect_error(linreg_mod <- linreg(formula = Petal.Length~Sepdsal.Width+Sepal.Length, data=irfsfdis))
})
...
test_that("print() method works", {
    ...
    # Commented out, cannot get method to pass this test...
